...
class QazapModelManufacturercategory extends JModelAdmin
{
	/**
	...
	*/
	 ...
	 * @since	1.0.0
	 */
	public function getTable($type = 'Manufacturercategory', $prefix = 'QazapTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_qazap.manufacturercategory', 'manufacturercategory', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_qazap.edit.manufacturercategory.data', array());
	...
	*/
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) 
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__qazap_manufacturercategories');
				$max = $db->loadResult();
	...
	public function save($data)
	{
		...
		try
		{
			...
			$result = $dispatcher->trigger('onBeforeSave', array('manufacturercategory', &$data, $isNew));			
			...
			$dispatcher->trigger('onAfterSave', array('manufacturercategory', $data, $isNew));
	...
	protected function generateNewTitleAlias($alias, $title, $language = null)
	{
		...
		$query = $db->getQuery(true)
					->select('COUNT(alias)')
					->from($db->quoteName('#__qazap_manufacturercategories'))
	...
	protected function checkData(&$data)
	{
		...
		if (trim($data['manufacturer_category_name'] == ''))
		{
		...
		if (trim($data['alias']) == '')
		{
			$data['alias'] = $data['manufacturer_category_name'];
		}
	...
	* @return
	*/	
	protected function getAliasExists($skipID = null)
	{
		$skipID = (int) $skipID;
		$db = $this->getDbo();
		$query = $db->getQuery(true)
