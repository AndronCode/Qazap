 ...
 * Qazap model.
 */
class QazapModelShipmentmethod extends JModelAdmin
{
	/**
	...
	*/
	 ...
	 * @since	1.0.0
	 */
	public function getTable($type = 'Shipmentmethod', $prefix = 'QazapTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_qazap.shipmentmethod', 'shipmentmethod', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_qazap.edit.shipmentmethod.data', array());
	...
	protected function mergePluginParams($data, $form)
	{
		...
		if(isset($data['shipment_method']) && $data['shipment_method'])
		{
			$plugin = QazapHelper::getPlugin($data['shipment_method']);
			$paramsFormFile = null;
	...
	public function save($data)
	{
		...
		$data['countries'] = (array) isset($data['countries']) ? $data['countries'] : array();
		$data['user_group'] = (array) isset($data['user_group']) ? $data['user_group'] : array();
		$data['shipment_method'] = (int) isset($data['shipment_method']) ? $data['shipment_method'] : 0;
		...
		if(empty($plugin))
		{
			$this->setError(JText::_('COM_QAZAP_SHIPMENTMETHOD_ERROR_INVALID_METHOD'));
			return false;				
		}
		
		// Trigger the onEventBeforeSave event.
		$result = $dispatcher->trigger('onBeforeSave', array('shipmentmethod', &$data, $isNew));
		...
		try
		{
			...
			if(!isset($table->shipment_method) && empty($table->shipment_method))
			{				
				$this->setError(JText::_('COM_QAZAP_SHIPMENTMETHOD_ERROR_INVALID_METHOD'));
				return false;
			}
			...
			$dispatcher->trigger('onAfterSave', array('shipmentmethod', $data, $isNew));
	...
	*/
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) 
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__qazap_shipment_methods');
				$max = $db->loadResult();
	...
	/**
	* Method to get Shipment Method Plugin Params
	* 
	* @return JForm object
	* @since	1.0.0
	*/
