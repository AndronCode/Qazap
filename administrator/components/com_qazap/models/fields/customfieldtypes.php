 ...
 */
class JFormFieldCustomfieldtypes extends JFormField
{
	/**
	 * The form field type.
	 *
	 ...
	 * @since	1.0.0
	 */
	protected $type = 'customfieldtypes';
	...
	protected function getInput()
	{
		...
		else 
		{
			...
			$attr = 'required="true" onchange="return getQazapCustomFieldParams(this);"';
	...
	protected function getOptions()
	{
		...
		$query = $db->getQuery(true)
					...
					->where('e.folder = '. $db->quote('qazapcustomfields'))
					->where('e.enabled = 1')
		...
		if(count($datas))
		{
			...
			{
				$options[] = JHtml::_('select.option', (int) $data->extension_id, $data->name);
			}				
		}
	...
	protected function getScripts()
	{
		...
		$doc->addScriptDeclaration("
			function getQazapCustomFieldParams(field) {
				var id = jq(field).val();
				if(id > 0) {
					jq.ajax({
						type: 'GET',
						dataType: 'html',
						url: window.qzuri,
						data: 'option=com_qazap&view=customfieldtype&format=raw&layout=params&plugin_id='+id,
						beforeSend: function() {
							Qazap.addloader(jq(field).parent());
						},
						success: function(data) {
							jq('#customFieldParams').html(data);
