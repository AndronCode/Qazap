 ...
 * @since       1.0.0
 */
class JFormFieldQazapShipmentParams extends JFormField
{
	...
	* @since  1.0.0
	*/
	protected $type = 'QazapShipmentParams';
	...
	protected function getInput()
	{
		...
		else 
		{
			...
			$attr = 'required="true" onchange="return getShipmentType(this);"';	
	...
	protected function getOptions()
	{
		...
		$query = $db->getQuery(true)
					...
					->where('e.folder = '. $db->quote('qazapshipment'))
					->where('e.enabled = 1')
	...
	protected function getScripts()
	{
		...
		$doc->addScriptDeclaration("
			function getShipmentType(field) {
				var id = jq(field).val();
				if(id > 0) {
					jq.ajax({
						type: 'GET',
						dataType: 'html',
						url: window.qzuri,
						data: 'option=com_qazap&view=shipmentmethod&format=raw&layout=params&plugin_id='+id,
						beforeSend: function() {
							Qazap.addloader(jq(field).parent());
						},
						success: function(data) {
							jq('#ShipmentParams').html(data);
