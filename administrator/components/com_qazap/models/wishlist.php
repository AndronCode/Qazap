...
*/
class QazapModelWishlist extends JModelList 
{
	...
	*/
    protected function getListQuery() 
	{
		...
		$query->select(
			$this->getState(
				'list.select', 'a.id, a.product_id, a.date'
			)
		);
		$query->from('`#__qazap_wishlist` AS a');
		$query->select('b.name, b.username, b.email, b.id AS user_id')
		...
		if($this->_multiple_language)
		{
			...
			$query->join('INNER', '#__qazap_product_details AS pd ON pd.product_id = a.product_id AND pd.language = '.$db->quote($this->_present_language));
			
		}
		else
		{
			$query->select('pd.product_name');
			$query->join('INNER', '#__qazap_product_details AS pd ON pd.product_id = a.product_id AND pd.language = '.$db->quote($this->_default_language));
		}
	...
	*/
	public function delete(&$pks)
	{
		...
		$sql = $db->getQuery(true)
				 ->delete($db->quoteName('#__qazap_wishlist'))
