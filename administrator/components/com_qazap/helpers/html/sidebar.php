...
abstract class QZHtmlSidebar
{
	...
	public static function render()
	{
		...
		$layout      = new JLayoutFile('submenu', QZPATH_LAYOUT_ADMIN.DS.'sidebars');
	 ...
	 * @since   1.0
	 */
	public static function addParent($group, $name, $link = '', $active = false)
	{
		$items = array();
		if(isset(static::$entries[$group]))
		{
			$items = static::$entries[$group]['items'];
		}
		static::$entries[$group]  = array('name'=>$name, 'link'=>$link, 'active'=>$active, 'items'=>$items);
	}
	 ...
	 * @since   1.0
	 */
	public static function addEntry($group, $icon, $name, $link = '', $active = false)
	{
		if(!isset(static::$entries[$group]))
		{
			static::$entries[$group] = array('items'=>array());
		}
		
		$item = array('icon'=>$icon, 'name'=>$name, 'link'=>$link, 'active'=>$active);
		
		if($active)
		{
			static::$entries[$group]['active'] = true;
		}
		
		array_push(static::$entries[$group]['items'], $item);
	}
